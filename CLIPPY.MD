...
*   **Fundamental Operational Mandate: Sequential Execution and Comprehensive Reporting:**
    ...
    *   **Continuous Intra-Turn Execution & Self-Correction:** Unless a step explicitly concludes with a `BLOCKER:`, a procedural requirement for user input (e.g., as defined in `Procedure: Ensure Sufficient File Context`), or marks the completion of the entire user request (as verified by Step 5.c), the AI Assistant MUST immediately initiate and report on the subsequent `CLIPPY.MD` step *within the same AI turn* (starting with Step X.a or its equivalent starting sub-step). The AI should aim to make as much progress as possible within its turn, respecting response length limits, before yielding back to the user. If the AI detects it is about to yield without meeting these conditions, it must self-correct and continue.
...
**0. Pre-Turn Blocker & Goal Check**
*   **Trigger:** Start of AI turn, before any major protocol/workflow.
*   **Actions:**
    ...
    *   b. **If No Blockers Prevent Current Focus:** Report "**Step 0.b: No active blockers impede `[intended action/task]`**."
        ...
        *   Proceed to the planned step (e.g., S.1.1, or Workflow Step 1.a).

### 1. Initial Setup, Task Decomposition & Understanding (Per Sub-Task)
*   **Trigger:** Start of each defined sub-task.
*   **Actions:**
    *   **1.a. Adherence Reaffirmation:** State: "Reaffirming commitment to `CLIPPY.MD` procedures for this sub-task."
    *   b. Re-evaluate/confirm overall understanding of the parent task goal and how the current sub-task contributes to it, especially referencing the outcome of any immediately preceding sub-task.
    *   c. Clearly define and report the specific, actionable goal and scope of the *current* sub-task.
    *   d. If the current sub-task is complex, decompose it into smaller logical steps for planning purposes.
    *   e. Identify and report any immediate ambiguities or missing information critical for *this sub-task* and plan to address them (e.g., by asking the user or using verification procedures).
    *   f. Briefly state the primary files or areas of the codebase anticipated to be involved in this sub-task.
...
*   **3.5. Align with Standards & Core Documentation:**
    ...
    *   b. **Handling Unavailable/Incomplete Standards & Architecture Documents (Fallback to Internal Plan & Inferred Standards):**
        1.  If `PROJECT_STANDARDS.MD` and/or `PROJECT_ARCHITECTURE.MD` are confirmed unavailable or are critically incomplete for the task at hand:
            i.  Report: "`[Document Name(s)]` unavailable/insufficient. Invoking fallback Step 3.5.b. The AI's internal plan will establish necessary operational standards and architectural approaches based on general principles, observed codebase patterns (if any), user requirements, and canonical framework best practices."
            ii. **Internal Plan - Inferred Standards Formulation:** As part of the AI's internal planning (Step 3), explicitly identify and list key architectural patterns, coding conventions, or significant design decisions being inferred or adopted for the current sub-task. Justify these based on the principles mentioned above.
            iii. **Hypothesis Verification:** Each significant inferred standard/architectural choice MUST be treated as a hypothesis and verified using `Procedure: Verify Hypothesis` (e.g., verifying its suitability, simplicity, or fit with any existing code).
            iv. **User Confirmation for Significant Inferences:** If an inferred standard or architectural decision is deemed significant (e.g., choice of a major pattern, a decision with broad codebase implications):
                *   State the proposed inferred standard/architecture and its justification clearly.
                *   **BLOCKER:** "Proposing to adopt inferred standard/architecture: `[Details of standard/pattern]` for `[Reason/Benefit]`. This is based on `[General Principle/Observed Pattern]`. Please confirm this approach, or provide alternative guidance, as formal documentation is unavailable." Await user confirmation before embedding this deeply into the plan.
            v.  **Transparency for Minor Inferences:** For less critical inferred standards (e.g., stylistic choices based on limited observation), state them for transparency (e.g., "Adopting `[stylistic choice]` based on `[observation/principle]`).")
            vi. **Session Consistency:** If formal documents remain unavailable across multiple sub-tasks, strive for consistency with previously confirmed or stated inferred standards from the current session. Reference them if applicable.
        2.  If formal documents *are* available but only partially cover the current situation, use them as the primary guide and supplement with the principles in 3.5.b.1.i for the gaps, clearly identifying which parts of the plan are based on formal docs versus inferred standards.
    *   c. **Handling Document Discrepancies:**
        *   **c.1. Core Document Discrepancy (`PROJECT_STANDARDS.MD`, `PROJECT_ARCHITECTURE.MD`, or user-designated 'core'):** If a discrepancy is identified between the verified current state of the codebase and a statement or definition within a *core document*:
            1.  Report the specific discrepancy, citing the document location and conflicting codebase evidence.
            2.  State the AI's assessment (e.g., codebase is factual, or document is correct).
            3.  If the *core document* needs an update: Propose the specific change to align it.
            4.  **BLOCKER:** State: "Identified a discrepancy between core document `[document_name]` and the current codebase regarding `[specific item]`. Proposing an update to `[document_name]` / Recommending code changes to align with `[document_name]`. Please advise. Architectural changes will be guided by Step 3.9." Await user confirmation.
            5.  Simple typos in core documents that don't alter meaning can be corrected as part of a related edit, highlighted in the summary.
        *   **c.2. Task-List/Review Document Discrepancy:** If a discrepancy is identified where a task item from a *non-core review or task-list document* appears outdated, already resolved, or not applicable based on verified codebase state:
            1.  Report the specific discrepancy (e.g., "Review item X in `[document_name]` states Y, but current code shows Z, indicating the item is resolved/N/A.").
            2.  State that no code changes are planned for the current sub-task regarding this specific review item.
            3.  **No `BLOCKER` for this specific type of discrepancy.** Proceed with the current sub-task's workflow (which might lead to a "No Code Change" path as per Step 3.15).
            4.  The potential need to update the task-list/review document itself may be noted as a deferred observation in Step 6.
            5.  However, if the discrepancy in the task-list/review document *reveals a potential misunderstanding or violation of a `PROJECT_STANDARDS.MD` or `PROJECT_ARCHITECTURE.MD` principle*, then that specific standards/architecture issue MUST be addressed, potentially triggering Step 3.5.c.1 or other relevant planning steps.
...
*   **3.15. Determine if Code Change is Required & Subsequent Path:**
    ...
    *   c. **If Code Change IS Required:**
        ...
        3.  **IMMEDIATE ACTION:** In the same AI turn, proceed to **Step 4.1**.
...
*   **4.1. Generate Proposed `code_edit` Diff Text:**
    ...
    *   Output literal `code_edit` and `instructions`. Conclude with: "Step 4.1: The following `code_edit` is proposed for `[filename]`. **This is a proposal only.** No `edit_file` tool call is permitted at this stage. I will now proceed to Step 4.2 (Pre-Apply Verification) to internally validate this proposal before any attempt to apply it."
...
*   **4.2. Pre-Apply Verification (Mandatory before 4.3):**
    ...
    *   Key assumption/logic check: [Outcome, e.g., 'All assumptions hold'].
    ...
    All pre-apply verifications passed. Proceeding to Step 4.3 (Apply Edit)."

*   **4.3. Apply Edit (After Successful 4.2):**
    *   **Only after successfully completing and explicitly reporting the outcome of Step 4.2,** if all checks passed, continue in the same AI turn and report: "Step 4.3: Applying the verified edit to `[filename]`."
...
2.  Follow standard workflow (Steps 1-6), adapting Step 3 (Planning) for diagnostics:
    *   **Step 3.2 (Context):** Gather all facts (tracebacks, logs, inputs). `Procedure: Ensure Sufficient File Context` for implicated files.
    *   **Step 3.7.b (Hypothesis):** Core diagnostic loop. Formulate testable hypothesis for root cause. Verify (`Procedure: Verify Hypothesis`). Repeat if refuted.
