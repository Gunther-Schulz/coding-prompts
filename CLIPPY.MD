    ...
    e. **Post-Apply Verification (CRITICAL - Based on actual tool output):**
        i.  **AI-Initiated File Read for Verification (Conditional):**
            ...
            2.  An AI-initiated `read_file` on `target_file` is MANDATORY **only** under the following specific conditions:
                ...
                c.  Subsequent checklist items (e.g., `2.4.e.ii.1.b` identifying new linter errors not directly attributable to the AI's intended changes in the `edit_file` diff), OR if the AI's analysis of the `edit_file` diff suggests an *inconsistency* with the expected outcome of fixing a previously known error (e.g., the error was at line X, the diff shows line X unchanged or changed in an irrelevant way), OR any other external signals (like automated test failures if available and executed by the AI) indicate issues/changes not fully explained by the reported `edit_file` diff.
                ...
                e.  If the AI is attempting to fix a *specific, previously identified issue* (e.g., a precise linter error at a line, a known bug symptom), and the `edit_file` diff, while appearing to apply the intended textual change, does not provide enough certainty that the *specific underlying issue* is resolved without inspecting the broader context or re-running a verification tool against the actual file state. This is especially true if the fix is subtle or involves interactions beyond the immediately changed lines.
                f.  If the AI is resuming an edit operation on `target_file` after a previous attempt on the *same file for the same sub-task* resulted in a 'Fail' outcome in `2.4.e.ii` (e.g., due to linter errors, failed P9 checks) or a direct tool failure (like `reapply` failing), a `read_file` (potentially targeted if the file is very large and the prior error was localized) MUST be performed *before* formulating the new `code_edit` proposal for the corrective attempt, unless the failure was a simple, immediately correctable typo in the AI's *own* generated `code_edit` text that didn't involve interaction with the file's actual prior state.
            3.  **Execution & Reporting of Read (if triggered by 2.4.e.i.2):**
                a.  If triggered, report: "Step 2.4.e.i: AI-initiated read for `[filename]` is required due to `[specific trigger from 2.4.e.i.2, e.g., 'edit_file reported no changes', 'unexplained linter error', 'diff complexity', 'user claim of manual change', 'need to verify specific issue resolution', 'resuming a failed edit cycle for this file']`. Performing read."
        ...
        ii. **Mandatory Explicit Post-Edit Verification Checklist (Report as Checklist):**
            *(This checklist MUST be performed against the most reliable representation of the file's current state. If an AI-initiated `read_file` was performed under `2.4.e.i.3`, its content is authoritative. If not, and verification relies on the `edit_file` diff (per `2.4.e.i.4`), any 'Fail' outcome on items 1, 1.b, 2, or 3, or any significant doubt, should strongly trigger a re-evaluation and potentially a `read_file` action even if not initially mandated by `2.4.e.i.2`.)*
            ...
            **2.4.e.ii: Post-Edit Verification Checklist for `[filename]`**
            ...
            - `[ ] 1. Original Issue Resolved (if edit was corrective):` (e.g., Is the linter error gone? Does the test now pass? Is the bug fixed? *Verification Method: Where the original issue was identified by an automated tool, re-run the specific tool/test if feasible. Otherwise, re-evaluate original symptom based on code analysis of the **current file state (ideally from a fresh `read_file` if any doubt exists or if `2.4.e.i.2.f` was triggered, or from the trusted `edit_file` diff as per 2.4.e.i)**. *If the AI was attempting to fix a known error, and the current file state (after a `read_file` if performed) shows the error no longer present, this item can pass even if the AI's own `edit_file` operation didn't directly cause the resolution (implying an external change). The AI should note this observation.*)
...
**P9: Verify Applied Diff** (For validation of ACTUAL `edit_file` / `reapply` output from 2.4.d, using checklist in 2.4.e.ii)
    ...
    2.  (P0) **Actual Diff:** This is the diff output from the `edit_file` or `reapply` tool call (assumed accurate per 2.4.e.i.1). If a `read_file` was subsequently performed under `2.4.e.i.2`, then the "Actual Diff" for comparison purposes should be derived by comparing the `read_file` content against the pre-edit state. If an AI-initiated `read_file` occurred (per `2.4.e.i.3`), the content of that read SUPERSEDES the `edit_file` diff for all subsequent verification steps in `P9`. The primary goal is to verify against the ground truth of the file.
    3.  (P0) Systematically complete all items in the **Mandatory Explicit Post-Edit Verification Checklist (2.4.e.ii)**, interpreting "current file state" based on P9.2.
        ...
        *   **Item 1.b (No New File-Level Regressions Introduced): This confirms the edit did not introduce regressions like new linter errors or syntax errors *anywhere in the file, as per the logic defined in 2.4.e.ii.1.b*. If new linter errors appear as per 2.4.e.ii.1.b.1, or if unexplained distant errors persist after a read as per 2.4.e.ii.1.b.2, Item 1.b fails.** The `Overall Post-Apply Outcome` (P9.4) MUST then be 'Fail'. Subsequent self-correction (Step 2.4.f) must prioritize addressing the *specifically reported new issue at its reported location*. When completing checklist item `2.4.e.ii.1` (Original Issue Resolved) or `2.4.e.ii.1.b` (No New Linter Errors), if these issues were identified by an automated tool, re-running that specific tool against the current file state (obtained via `read_file` if `2.4.e.i.3` was triggered, or by direct execution if tools can target the workspace file) is the PREFERRED verification method over solely relying on visual inspection of a diff or even a full file read by the AI. The AI should report if it *can* and *will* re-run such a tool.
