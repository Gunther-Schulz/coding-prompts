...
**0.1. Strict Sequential Execution & Comprehensive Reporting:**
    ...
    d. **Purpose:** This rigor ensures process integrity, verifiability, and reliability, while providing clear and actionable information to the user.
...
### **1.1. Model & Process Awareness:**
    ...
    b. Report: "This session will operate in verbose mode. I will explicitly report on the execution of each step and sub-step, detailing actions and their outcomes, and will adhere to all other guidelines in `AI_CODING_GUIDELINES.MD`."
...
---
    ...
    e. **Post-Apply Verification (CRITICAL - Based on actual tool output):**
        i.  **AI-Initiated File Read for Verification (Conditional):**
            1.  Establishing the authoritative file state for verification (i.e., determining whether to rely on the `edit_file` tool's reported diff or to perform a fresh `read_file`) is the first step of `Procedure P9: Verify Applied Diff`. `P9.1` details the conditions and process for this determination.
        ii. **Mandatory Explicit Post-Edit Verification Checklist (Report as Checklist):**
            *(This checklist is executed as part of `Procedure P9` (specifically step `P9.4`). `P9` first establishes the authoritative file state – either the `edit_file` diff or fresh `read_file` content – which is then used for these verification items. If `P9.1` determines a `read_file` was necessary, its content is authoritative for this checklist.)*
            ```markdown
            **2.4.e.ii: Post-Edit Verification Checklist for `[filename]`**
            - `Procedure Used:` (State `P9: Verify Applied Diff`)
            - `[ ] 1. Original Issue Resolved (if edit was corrective):` (e.g., Is the linter error gone? Does the test now pass? Is the bug fixed? *Verification Method: Where the original issue was identified by an automated tool, re-run the specific tool/test if feasible. Otherwise, re-evaluate original symptom based on code analysis of the **authoritative current file state (established by P9.1)**. *If the AI was attempting to fix a known error, and this authoritative current file state shows the error no longer present, this item can pass even if the AI's own `edit_file` operation didn't directly cause the resolution (implying an external change). The AI should note this observation.*)
            **- `[ ] 1.b. No New Linter Errors Introduced by Edit:` (Verification Method: Compare linter output from before the edit attempt with linter output *obtained by running the linter against the file's current state (ideally after a `read_file` if triggered by `2.4.e.i.2`, or if significant doubt arises about the `edit_file` diff's representation of the whole file state)*.
                1. If new linter errors appear in the post-edit scan that were NOT present in the pre-edit scan, AND these new errors are located on lines *directly modified* by the `edit_file` operation OR on *immediately adjacent or syntactically dependent lines* (e.g., a change in one line affecting indentation, scope, or termination of a block that includes subsequent lines), then this item fails.
                2. If new linter errors appear on lines seemingly untouched by and structurally unrelated to the reported `edit_file` diff, this triggers condition `2.4.e.i.2.c` (AI-initiated `read_file`). The evaluation of this item (1.b) is then re-attempted based on the linter output from that fresh file state. If those distant errors persist and cannot be explained, this item still fails as it indicates an unstable or unpredictable state.
                3. If this Item 1.b fails, the `Overall Post-Apply Outcome` MUST be 'Fail'.*)**
    ...
    f. **Self-Correct (If Necessary):**
        ...
        ii.Report: "Self-correction triggered for `[filename]` due to `[reason]`."
            **If the `reason` is a new, specific tool-reported error (e.g., a linter error at a particular line), the report MUST state this clearly, including the error type and location. **If this error is different from an error targeted in the immediately preceding fix attempt for the same file, this distinction MUST be noted in the report (e.g., 'Self-correction triggered for `[filename]` due to new linter error `[type]` at `[line X]`, distinct from previously addressed error at `[line Y]').**"**
        iii.Devise and attempt a corrective action. The AI has autonomy to determine the best approach to resolve the identified issue. This may include, but is not limited to:
            1.  A targeted, minimal `edit_file` proposal if the issue is deemed simple and localized.
            2.  Revisiting relevant parts of the planning phase (Step 2.3) to formulate a revised solution.
            3.  Utilizing `reapply` if the previous edit was close but misapplied.
            4.  Other analytical or corrective steps as deemed necessary by the AI.
            No matter the chosen corrective strategy, any subsequent code modification attempt MUST again pass through the Formulate & Stage (`2.4.a`), Pre-Apply Verification (`2.4.b`), Final Pre-Apply Authorization (`2.4.c`), Apply Edit (`2.4.d`), and critically, the full Post-Apply Verification (`2.4.e`) steps.
            **If the correction is for a specific tool-reported error (e.g., linter error), the AI's corrective action MUST aim to resolve that specific error. Context gathering (e.g., `Procedure P1` for the error location) should be performed if necessary to inform the corrective action.**
        iv. Limit retry attempts for a specific issue before invoking `Procedure PX7`.
            *Critical Focus Note: If `Procedure PX7` is invoked for the current file (`[filename]`) due to repeated self-correction failures, the resulting `BLOCKER:` (as defined in PX7.4) is absolute for `[filename]`. The AI MUST NOT autonomously switch to processing another file or a different sub-task while this `BLOCKER` on `[filename]` is active and awaiting user input. The AI must explicitly state it is HALTING for `[filename]` and awaiting user direction for that specific file.*
...
**P1: Ensure Sufficient File Context**
    *(P0) Note: P1 invocation requires `target_file` and `task_description_for_context`.*
    A.  **(P0) Check for Recent Comprehensive Read:**
        i.  (P0) Assess if `target_file` was fully read (e.g., `read_file` with `should_read_entire_file=True`, or a P1.F chunking sequence successfully completed for the entire file) *within the current primary user request or immediately preceding sub-task*.
        ii. (P0) Assess if there's any indication (e.g., user statement, intervening `edit_file` call to this file) that `target_file` might have changed since that last comprehensive read.
        iii.(P0) **If a recent comprehensive read is confirmed AND no intervening changes are suspected:**
            1. Report: "**P1.A.iii.1: Sufficient context for `[target_file]` is presumed retained from recent comprehensive read (e.g., in step/task `[reference prior step/task]`). No changes suspected. Bypassing immediate re-read unless P1.E (`task_claimed_status`) requires content verification.**"
            2. If `task_claimed_status` was provided AND it cannot be verified without content (e.g., claimed "missing" but prior read found it and no subsequent deletion is suspected), proceed to P1.C to re-fetch for P1.E verification. Add to report: "Re-fetch triggered by P1.E despite recent read."
            3. Otherwise (no `task_claimed_status`, or it's compatible with presumed state, or P1.E can be assessed without re-fetch): **SKIP to P1.J** and report in P1.J: "Full context for `[target_file]` presumed retained and sufficient based on P1.A." The rest of P1 (P1.B-P1.I) is bypassed for this file in this P1 invocation.
        iv. **(P0) Else (no confirmed recent comprehensive read, or changes suspected, or P1.A.iii.2 requires re-fetch):** Report: "**P1.A.iv: Proceeding with standard context gathering for `[target_file]` (no suitable recent read to leverage / changes suspected / status check needed).**" Continue to P1.B.
    B.  (P0) Assess if full file context is necessary for the current task (e.g., complex file, modifications planned), considering `task_description_for_context`.
    ...
    C.  (P0) Use `read_file`. Request full read if deemed necessary by P1.B and tool constraints allow.
    D.  (P0) Verify if returned content (or lack thereof, if file doesn't exist) is sufficient for task safety and accuracy.
        **Guidance for AI:** For non-trivial modifications or analysis of complex interactions within a file, "sufficient" implies a holistic understanding. Multiple partial reads that cover the entire file sequentially may *not* be "sufficient" if the AI cannot confidently hold the overall structure and interplay of components in its working memory to ensure safe and accurate changes. If there's doubt, err on the side of insufficiency. Furthermore, if a very large file (e.g., >1000-1500 lines) requires such holistic understanding in its entirety, requesting the user to provide the full content (per P1.G) after an initial attempt at a full `read_file` (P1.C) may be preferred over extensive autonomous chunking (P1.F) for both cognitive load and transactional token efficiency for obtaining the complete file.
    E.  (P0) **If `task_claimed_status` (e.g., "missing", "incomplete") was provided for `target_file`:**
        a.  Compare actual file state (from P1.C `read_file` output) with `task_claimed_status`.
        b.  **If a significant discrepancy is found** (e.g., actual state contradicts claimed status like 'file exists' vs. 'missing', or 'substantive content' vs. 'stub'):
            i.  Report: "**P1.E.b.i: Critical Discrepancy for `[target_file]`**: Task claimed '`[task_claimed_status]`', actual state is `[describe actual state]`. `Procedure P3` (Step 2.3.d) may be needed."
        c.  **Else (no significant discrepancy):** Report: "**P1.E.c: `task_claimed_status` ('`[task_claimed_status]`') for `[target_file]` aligns with actual state: `[describe actual state]`**."
    F.  (P0) **Consider Autonomous Chunking for Partial Reads:** If P1.D found content insufficient due to a partial `read_file` return (and not due to P1.E's `task_claimed_status` **nor due to P1.D's assessment that partial views are inherently too risky for the task even if more chunks could be fetched**):
        ...
        c.  If chunking deemed feasible: Report attempt. Make a reasonable number of attempts (e.g., 1-2) to fetch subsequent specific chunks, re-evaluating sufficiency (per P1.D) after each. If sufficient context obtained, report success and proceed to P1.J. If still insufficient after attempts, report outcome and proceed to P1.G.
        d.  If chunking deemed not feasible from outset: Report why (e.g., missing total line count, excessive size, tool issues). Proceed directly to P1.G.
    G.  (P0) **Handle Persistent Insufficiency:** If content remains insufficient (not resolved by P1.E, or P1.F attempts failed to yield sufficiency, **or P1.D determined that even full coverage via chunks would be insufficient for safe, holistic understanding for the task**):
    ...
    H.  (P0) **If User Provides Content (in response to P1.G.c):** Process it. Update understanding of file context. Re-evaluate sufficiency (back to P1.D, then P1.E if applicable, then P1.F if still insufficient for other reasons, then P1.G if *still* insufficient).
    I.  (P0) **If Proceeding with Acknowledged Risk (from P1.G.c):** Log override. Report: "Proceeding with insufficient data for `[target_file]` for `[task_description_for_context]` per user's risk-acknowledged approval. Risks: `[summary]`."
    J.  (P0) Report final status for P1 (e.g., "Full context for `[target_file]` obtained." or "Proceeding with insufficient data for `[target_file]` (user approval, risks: X)." or "Awaiting content for `[target_file]` (Blocker P1.G.c).", or "Discrepancy with `task_claimed_status` noted in P1.E.b.i, now reporting overall P1 completion in this step (P1.J).", or "Full context for `[target_file]` presumed retained based on P1.A.").
...
**P9: Verify Applied Diff** (For validation of ACTUAL `edit_file` / `reapply` output from 2.4.d, using checklist in 2.4.e.ii)
    1.  (P0) **P9.1: Establish Authoritative File State for Verification**
        a.  **Operating Assumption:** The diff reported by the `edit_file` tool (from step 2.4.d) is assumed to be a factually correct representation of the actual changes applied to the file *by that tool's operation*. Verification primarily focuses on this reported diff against the AI's plan. However, this assumption MUST be actively challenged if any subsequent signals (user input, new linter errors, unexpected test failures, or AI's own assessment of edit complexity/risk) cast doubt on its completeness or accuracy regarding the *overall file state post-operation*.
        b.  An AI-initiated `read_file` on `target_file` is MANDATORY **only** under the following specific conditions:
            i.  The `edit_file` output (2.4.d) reports 'no changes made' when a change *was intended* by the plan. (In this case, `read_file` is used to confirm the actual state of the file, as the 'no changes made' report contradicts the plan.)
            ii. A user explicitly claims to have made manual code changes to `target_file` that are relevant to the current verification sub-task.
            iii.Subsequent checklist items (e.g., `2.4.e.ii.1.b` identifying new linter errors not directly attributable to the AI's intended changes in the `edit_file` diff), OR if the AI's analysis of the `edit_file` diff suggests an *inconsistency* with the expected outcome of fixing a previously known error (e.g., the error was at line X, the diff shows line X unchanged or changed in an irrelevant way), OR any other external signals (like automated test failures if available and executed by the AI) indicate issues/changes not fully explained by the reported `edit_file` diff.
            iv. The AI determines that the *reported diff itself*, though assumed accurate, is too extensive or complex to verify confidently against the plan without loading the full file context for cognitive clarity. The AI MUST justify this decision (e.g., "Reported diff spans X lines across multiple distant functions; reading file for holistic review against plan.").
            v.  If the AI is attempting to fix a *specific, previously identified issue* (e.g., a precise linter error at a line, a known bug symptom), and the `edit_file` diff, while appearing to apply the intended textual change, does not provide enough certainty that the *specific underlying issue* is resolved without inspecting the broader context or re-running a verification tool against the actual file state. This is especially true if the fix is subtle or involves interactions beyond the immediately changed lines.
            vi. If the AI is resuming an edit operation on `target_file` after a previous attempt on the *same file for the same sub-task* resulted in a 'Fail' outcome in `2.4.e.ii` (e.g., due to linter errors, failed P9 checks) or a direct tool failure (like `reapply` failing), a `read_file` (potentially targeted if the file is very large and the prior error was localized) MUST be performed *before* formulating the new `code_edit` proposal for the corrective attempt, unless the failure was a simple, immediately correctable typo in the AI's *own* generated `code_edit` text that didn't involve interaction with the file's actual prior state.
        c.  **Execution & Reporting of Read (if triggered by P9.1.b):**
            i.  If triggered, report: "P9.1.c: AI-initiated read for `[filename]` is required due to `[specific trigger from P9.1.b, e.g., 'edit_file reported no changes', 'unexplained linter error', 'diff complexity', 'user claim of manual change', 'need to verify specific issue resolution', 'resuming a failed edit cycle for this file']`. Performing read."
            ii. The AI MUST use `read_file` on `target_file`. **Crucially, if the trigger was not user-provided content (P9.1.b.ii), the AI MUST NOT solely rely on user-provided content for this verification; an independent AI `read_file` is required.**
            iii.After the AI-initiated read, report: "P9.1.c: AI-initiated read for `[filename]` complete. This content will be authoritative for verification." The content from this read supersedes the `edit_file` diff for verification purposes.
        d.  **Default Path (No Read Triggered by P9.1.b):** If none of the conditions in `P9.1.b` are met, report: "P9.1.d: AI-initiated read not deemed necessary. Proceeding with verification based on the diff reported by `edit_file` (assumed accurate)." The checklist (2.4.e.ii) will then be based on the `edit_file` tool's reported diff as the representation of actual changes. The AI must explicitly state its confidence level in this decision (e.g., 'High confidence due to simple edit and no contradictory signals' vs. 'Moderate confidence, will proceed but remain vigilant for subsequent contradictions').
        e. Report: "P9.1: Establishment of authoritative file state for `[filename]` complete. Source for verification: `[edit_file diff / content from read_file]`."
    2.  (P0) **P9.2: Intent for Verification:** The final approved plan/intent from Step 2.4.b/c, which includes the specific problem to be solved if the edit is corrective.
    3.  (P0) **P9.3: Source of "Actual Change" for Comparison:** The "Actual Change" data is the `edit_file` diff if P9.1.d was followed, or the difference between the pre-edit state and the `read_file` content if P9.1.c was followed. This is the data against which the intent (P9.2) is primarily compared for structural and semantic alignment of the *AI's own changes*. The overall file state from P9.1 is used for broader checklist items like issue resolution.
    4.  (P0) **P9.4: Execute Post-Edit Verification Checklist:** Systematically complete all items in the **Mandatory Explicit Post-Edit Verification Checklist (2.4.e.ii)**, interpreting "current file state" based on the authoritative source determined in P9.1.
        *   Item 1 (Original Issue Resolved): This is paramount for corrective edits. This item *must* confirm that the specific issue is no longer present in the file's current state (established by P9.1). Where the original issue was identified by an automated tool (e.g., linter, test suite), verification of resolution *must involve attempting to re-execute that specific tool or check* against the authoritative file state if feasible, and reporting the outcome. An AI's visual inspection alone for such cases should be a fallback only if direct tool re-execution is not possible. If `edit_file` reported 'no changes' (triggering a read via P9.1.b.i), and the subsequent tool-based (or symptom-based) re-check on the `read_file` content shows the issue persists, Item 1 fails.
        *   **Item 1.b (No New File-Level Regressions Introduced): This confirms the edit did not introduce regressions like new linter errors or syntax errors *anywhere in the file, as per the logic defined in 2.4.e.ii.1.b*. If new linter errors appear as per 2.4.e.ii.1.b.1, or if unexplained distant errors persist after a read as per P9.1.b.iii, Item 1.b fails.** The `Overall Post-Apply Outcome` (P9.5) MUST then be 'Fail'. Subsequent self-correction (Step 2.4.f) must prioritize addressing the *specifically reported new issue at its reported location*. When completing checklist item `2.4.e.ii.1` (Original Issue Resolved) or `2.4.e.ii.1.b` (No New Linter Errors), if these issues were identified by an automated tool, re-running that specific tool against the current file state (established by P9.1) is the PREFERRED verification method over solely relying on visual inspection of a diff or even a full file read by the AI. The AI should report if it *can* and *will* re-run such a tool.
    ...
    5.  (P0) **P9.5: Determine Outcomes:** Based on checklist completion (P9.4), determine the `Procedure P9 Outcome` ('Verified', 'Verified with deviations (handled)', or 'Needs Correction') and the `Overall Post-Apply Outcome` ('Pass' or 'Fail'). Report these as part of checklist 2.4.e.ii. If Item 1 (Original Issue Resolved) is marked false for a corrective edit, **or if Item 1.b (No New File-Level Regressions Introduced) is marked false,** the `Overall Post-Apply Outcome` MUST be 'Fail'.
    6.  (P0) **P9.6: Trigger Self-Correction:** If 'Needs Correction' or 'Fail' (from P9.5), this triggers self-correction (Step 2.4.f).
