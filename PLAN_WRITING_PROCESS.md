...
**Version: 1.1** (Language-Agnostic Refactoring)
...
*   **Action:**
    ...
    3.  **Read Current State:** **CRITICAL:** *Before* proposing any specific steps, **MUST** read the relevant sections (or potentially full files if small and central) of the key modules identified in 1.2 using `read_file`. **State explicitly** which files/sections were read. *Purpose: To base the plan on verified current code, not assumptions.* *Example: "Reading current state of `module_a.ext` (functions foo, bar) and `module_b.ext` (class Baz) to inform planning."*
        *   **NEW REQUIREMENT:** When the scope involves refactoring existing logic blocks, 'Reading Current State' **MUST** include explicitly documenting the **core computational goal and high-level algorithm/decision points, including all significant conditional branches, the specific conditions triggering them, and the distinct logic executed within each branch** of the logic being targeted for replacement. **This analysis MUST trace the complete execution path within the targeted function/method for all significant conditional branches (e.g., based on flags, error states, specific data conditions) to identify all potential outcomes and side effects (e.g., specific return values, exceptions raised, data store interactions, logging actions, external interactions) associated with the logic being replaced.**
        *   **NEW REQUIREMENT:** Analysis **MUST** also include explicitly documenting the **current roles and relationships** of the key existing components identified (e.g., how data model `X` currently links component `Y` to `Z`). Avoid designing target architecture elements until this existing structure is understood and documented.
...
*   **Action:** Draft the plan steps while performing the following analysis:
    ...
    **MANDATORY DEEP ANALYSIS FOR REFACTORING (Steps 1.3, 2.4.b, 2.9):**
    ...
    1.  **Verify Standards Alignment:** For each major part of the plan, explicitly reference the relevant principle or section from the project's standards document (`STANDARDS.md`) it adheres to. *Example: "Step 3 aligns with project standards (Error Handling section) by using central exceptions."*
    ...
    4.  **Ensure Clean, Forward-Looking Solutions:**
        **NEW Step 2.3.1: Generate Dependency Validation Summary (After Step 2.3 Checks):** Before proceeding, **MUST** generate a **Dependency Validation Summary**. This confirms key checks from Step 2.3 were performed. Structure using the following format:
            *   `[x/-] 1. Dependency Path/Symbol Checks:` *[Narrative confirming 2.3[ ] (Adding Dependencies - Path/Symbol Validation) performed for relevant references. Mark `[x]` if done, `[-]` if N/A.]*
            *   `[x/-] 2. Dependency Circularity Checks:` *[Narrative confirming 2.3[ ] (Adding Dependencies - Circularity Check) performed for relevant references. Mark `[x]` if done, `[-]` if N/A.]*
