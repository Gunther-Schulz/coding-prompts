...
*   **Action:**
    1.  **Check for Blocked Context:** Determine if this Step 3 planning cycle was initiated because a previous Step 4 edit attempt for the same overarching task was declared "blocked" (as per Step 4.4.3.b.e.i). The AI should have access to the reasons and history of the prior blocked attempt.
    2.  **If Prior Edit Attempt Was Blocked:**
        ...
        b.  **Consider Corrective Strategy (e.g., Refactoring):**
            i.  If the diagnosis (from 3.0.0.2.a) suggests that a specific corrective strategy, such as refactoring the problematic code block, is a viable path to unblock the original goal, then the AI **SHOULD** formulate a new, preliminary plan for this corrective action (e.g., a refactoring plan).
...
In all such cases where the tool's changes significantly exceed or deviate from the precise, narrow intent, it must be treated as a tool misapplication, even if some unintended changes seem minor. **This includes being skeptical of what the diff *doesn't* show. If an intended change, such as a deletion, is not explicitly confirmed by the diff, assume it may not have happened and verify directly (see `Procedure: Verify Diff`, Step 7).**
    ...
    *   **Action:** Based on the verified plan from Step 3, formulate the `code_edit` content (the proposed diff text) and associated `instructions`.
        ...
        *   **Output of this Step (Literal Text Presentation):** The AI **MUST** output the *literal text content* of the formulated `code_edit` string and the `instructions` string directly in its response. This is for textual presentation only. **This means showing the raw `code_edit` (e.g., `// ... existing code ...\nnew_line();\n// ... existing code ...`) and `instructions` strings themselves, NOT the diff that would result from applying them.** This textual presentation of the *proposed* edit content is for the AI's *own subsequent internal verification in Step 4.2*.
...
*   **Steps:**
    ...
    3.  **Verify Full Content Receipt & Handle Incompleteness:**
        ...
        c.  Action on Incomplete Read:
            ...
            ii.  The AI MUST then request the user to provide the full file content. The AI MUST clearly state the specific risks of proceeding without the full content if a complete view was deemed necessary in Step 1.
            iii. **BLOCKER:** The AI MUST NOT proceed with planning or generating edits for this file based on the incomplete information (if a complete view was deemed necessary in Step 1) UNTIL either:
                ...
                (b) The user, after being informed of the specific risks (as per sub-step ii), explicitly instructs the AI to proceed despite those risks.
